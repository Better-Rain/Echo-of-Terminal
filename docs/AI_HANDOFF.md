...
- 文件管理器和档案记录侧栏的搜索框默认保持空白，不预填提示搜索词。
...
10. 登录后稍等会出现 `secure-comm` 未投递消息提示；打开 B 的未投递缓存会话可看到旧消息说明账号原主人 A 当年未读，并提示先找 `RECORDS/signal_index_061.arc`。
11. 本地磁盘包含当前登录日志和历史损坏日志，集中在 `DISK://LOCAL/SYSTEM/LOGS/`。
12. 本地 `RECORDS/signal_index_061.arc` 和 `RECORDS/CASES/` 包含 `.arc` 档案源文件；文件管理器中显示为密文/乱码，档案记录区负责解析成可读档案。
13. 本地 `RECORDS/signal_index_061.arc` 解析为 `RX-061 / 短波接收索引`，指向 `rx-shortwave`；`signal_log_061.arc` 在档案记录中解析为短波接收日志，给出中心频率、项目代号和 `phase_jan.arc` 相位偏移表线索。
14. 在文件管理器搜索 `phase` 可找到 `SYSTEM/INDEX/phase_jan.arc`。
15. 在档案记录中打开 `RX-PHASE-JAN / 一月载波相位偏移表`，通过表格查到 `JANUS` 对应 `1.319 rad`。
16. 顶部导航可在 `文件管理器` 和 `档案记录` 之间切换。
17. `档案记录` 打开档案工作区，右侧标签可切换通信软件、短波接收器和时钟。
18. 在短波接收器中设置中心频率 `6.107 MHz`，将相位校准调到 `1.319 rad`。
19. 接收器锁定后显示 `REPORT_NORTHLINE_061.enc` 和密钥格式。
20. 回到文件管理器搜索 `REPORT_NORTHLINE_061`，打开加密报告。
21. 用同步签名 `RX-SIG-6149` 生成校验码 `6149`，提交密钥 `JANUS-0719-6149`。
22. 报告解锁后显示北线坐标报告正文，并标记本次短波谜题完成。
23. 可根据报告正文的诊断拨号码格式 `#*#*xxxx*#*#` 推出暗码 `#*#*6104*#*#`，在文件管理器查询栏执行后，系统会恢复此前不在普通目录枚举中的 `H-19_seat_residue.arc`，档案记录区同步出现 `H-19 / A席位残留索引`。
...
- 玩家进度没有持久化；刷新后报告解锁状态和隐藏文档发现状态都会丢失。
