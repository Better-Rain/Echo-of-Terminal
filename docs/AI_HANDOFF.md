...
- 第一阶段登录流程可玩：启动自检含授时中心三次重连失败、本地缓存时间接管和进度条暂停续走 -> 只读核心席位镜像账号 `中枢席位#0719` -> 口令 `0719` -> 身份验证反馈 -> 档案终端。
...
- 登录进入档案终端后约 7.2 秒会恢复一条沈德文发给中枢席位原持有人的旧消息。该消息以未投递队列形式注入 `shen-dewen` 会话，说明原持有人因未登录而没有读到，并引导玩家去 `RECORDS/signal_index_061.arc` 查接收索引。
...
10. 登录后稍等会出现 `secure-comm` 未投递消息提示；打开沈德文会话可看到旧消息说明中枢席位原持有人当年未读，并提示先找 `RECORDS/signal_index_061.arc`。
...
- `src/data/player.ts`：当前玩家档案。默认是旧系统核心席位 `中枢席位#0719` 的降级只读镜像；权限等级和权限列表暂不提升。
...
- 登录谜题仍然很简单，后续可替换。
- 核心席位原持有人姓名和头像仍是占位；当前只确定 0719 属于旧系统核心席位之一。
